...
namespace Server.Multis
{
    ...
    public abstract class BaseBoat : BaseMulti
    {
        ...
        public Point3D GetRotatedLocation(int x, int y)
        {
            ...
            return this.Rotate(p, (int)m_Facing / 2);
        }
        ...
        public void UpdateComponents()
        {
            ...
            {
                this.m_PPlank.MoveToWorld(this.GetRotatedLocation(PortOffset.X, PortOffset.Y), this.Map);
                this.m_PPlank.SetFacing(this.m_Facing);
            }

            if (this.m_SPlank != null)
            {
                this.m_SPlank.MoveToWorld(this.GetRotatedLocation(StarboardOffset.X, StarboardOffset.Y), this.Map);
        ...
        public bool Turn(int offset, bool message)
        {
            ...
            else if (this.SetFacing((Direction)(((int)m_Facing + offset) & 0x7)))
            {
                return true;
            }
            else
        ...
        public bool Move(Direction dir, int speed, bool message)
        {
            ...
            {
                if (!this.CanFit(new Point3D(X + (i * rx), Y + (i * ry), Z), Map, ItemID))
                {
                    if (i == 1)
                    {
                        if (message && this.m_TillerMan != null)
            ...
            for (int i = 0; i < wrap.Length; ++i)
            {
                ...
                if (rect.Contains(new Point2D(this.X, this.Y)) && !rect.Contains(new Point2D(newX, newY)))
                {
                    ...
                    {
                        if (!this.CanFit(new Point3D(newX + (j * rx), newY + (j * ry), Z), Map, ItemID))
                        {
                            if (message && this.m_TillerMan != null)
                                this.m_TillerMan.Say(501424); // Ar, we've stopped sir.
            ...
            if (this.Map != Map.Internal)
            {
                switch ( facing )
                {
                    case Direction.North:
                        if (!this.CanFit(Location, Map, NorthID))
                            return false;
                        break;
                    case Direction.East:
                        if (!this.CanFit(Location, Map, EastID))
                            return false;
                        break;
                    case Direction.South:
                        if (!this.CanFit(Location, Map, SouthID))
                            return false;
                        break;
                    case Direction.West:
                        if (!this.CanFit(Location, Map, WestID))
                            return false;
                        break;
                }
            }
