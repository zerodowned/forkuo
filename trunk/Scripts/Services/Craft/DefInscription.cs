...
namespace Server.Engines.Craft
{
    public class DefInscription : CraftSystem
    {
        ...
        private void AddSpell(Type type, params Reg[] regs)
        {
            ...
            int index = this.AddCraft(type, 1044369 + m_Circle, 1044381 + m_Index++, minSkill, maxSkill, m_RegTypes[(int)regs[0]], 1044353 + (int)regs[0], 1, 1044361 + (int)regs[0]);

            for (int i = 1; i < regs.Length; ++i)
                this.AddRes(index, m_RegTypes[(int)regs[i]], 1044353 + (int)regs[i], 1, 1044361 + (int)regs[i]);

            this.AddRes(index, typeof(BlankScroll), 1044377, 1, 1044378);

            this.SetManaReq(index, m_Mana);
        }
