        ...
        public override void OnThink()
        {
            if (!this.m_ToDelete && !this.Frozen)
            {
                ...
                if (!this.m_Necromancer.Alive)
                {
                    ...
                    this.Say(1060139, m_Necromancer.Name); // You have made my work easy for me, ~1_NAME~.  My task here is done.

                    this.m_ToDelete = true;

                    Timer.DelayCall(TimeSpan.FromSeconds(5.0), new TimerCallback(Delete));
                }
                else if (this.m_Necromancer.Map != this.Map || this.GetDistanceToSqrt(m_Necromancer) > this.RangePerception + 1)
                {
