...
namespace Server.Spells.Fourth
{
    public class ArchProtectionSpell : MagerySpell
    {
                    ...
                    if (targets.Count > 0)
                    {
                        for (int i = 0; i < targets.Count; ++i)
                        {
                            ...
                            if (m.BeginAction(typeof(ArchProtectionSpell)))
                            {
                                ...
                                new InternalTimer(m, this.Caster, val).Start();
        ...
        private class InternalTimer : Timer
        {
            ...
            {
                this.m_Owner.EndAction(typeof(ArchProtectionSpell));
                this.m_Owner.VirtualArmorMod -= this.m_Val;
                if (this.m_Owner.VirtualArmorMod < 0)
                    this.m_Owner.VirtualArmorMod = 0;
            }
        }
