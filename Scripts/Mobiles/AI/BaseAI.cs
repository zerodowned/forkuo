...
namespace Server.Mobiles
{
    ...
    public abstract class BaseAI
    {
        ...
        public virtual bool DoActionWander()
        {
            ...
            else if (this.m_Mobile.CurrentWayPoint != null)
            {
                ...
                if ((point.X != this.m_Mobile.Location.X || point.Y != this.m_Mobile.Location.Y) && point.Map == this.m_Mobile.Map && point.Parent == null && !point.Deleted)
                {
                    ...
                    this.DoMove(m_Mobile.GetDirectionTo(m_Mobile.CurrentWayPoint));
                }
            ...
            else if (this.m_Mobile.IsAnimatedDead)
            {
        ...
        public virtual bool DoActionFlee()
        {
            ...
            if (this.WalkMobileRange(from, 1, true, m_Mobile.RangePerception * 2, m_Mobile.RangePerception * 3))
            {
        ...
        public virtual bool DoOrderCome()
        {
            if (this.m_Mobile.ControlMaster != null && !this.m_Mobile.ControlMaster.Deleted)
            {
                ...
                else
                {
                    ...
                    if (this.WalkMobileRange(m_Mobile.ControlMaster, 1, bRun, 0, 1))
                    {
                        if (this.m_Mobile.Combatant != null && !this.m_Mobile.Combatant.Deleted && this.m_Mobile.Combatant.Alive && !this.m_Mobile.Combatant.IsDeadBondedPet)
                        {
                            this.m_Mobile.Warmode = true;
        ...
        public virtual bool CheckHerding()
        {
            ...
            this.DoMove(m_Mobile.GetDirectionTo(target));
        ...
        public virtual bool DoOrderFollow()
        {
            ...
            else if (this.m_Mobile.ControlTarget != null && !this.m_Mobile.ControlTarget.Deleted && this.m_Mobile.ControlTarget != this.m_Mobile)
            {
                ...
                else
                {
                    ...
                    if (this.WalkMobileRange(m_Mobile.ControlTarget, 1, bRun, 0, 1))
                    {
                        if (this.m_Mobile.Combatant != null && !this.m_Mobile.Combatant.Deleted && this.m_Mobile.Combatant.Alive && !this.m_Mobile.Combatant.IsDeadBondedPet)
                        {
                            this.m_Mobile.Warmode = true;
                ...
                if (Utility.Random(8 * iChanceToNotMove) <= 8)
                {
                    ...
                    switch( iRndMove )
                    {
                        ...
                        case 8:
                            ...
                            break;
                        default:
                            this.DoMove(m_Mobile.Direction);
                            break;
                    }
                }
            }
        ...
        public virtual MoveResult DoMoveImpl(Direction d)
        {
            ...
            TimeSpan delay = TimeSpan.FromSeconds(this.TransformMoveDelay(m_Mobile.CurrentSpeed));
            ...
            if (this.m_Mobile.Home == Point3D.Zero)
            {
                if (this.m_Mobile.Spawner is SpawnEntry)
                {
                    ...
                    else
                    {
                        ...
                        {
                            this.DoMove(m_Mobile.GetDirectionTo(region.GoLocation));
                        }
                        else
                        {
            ...
            else
            {
                for (int i = 0; i < iSteps; i++)
                {
                    if (this.m_Mobile.RangeHome != 0)
                    {
                        ...
                        {
                            this.DoMove(m_Mobile.GetDirectionTo(m_Mobile.Home));
                        }
                        else
                        {
        ...
        public virtual bool MoveTo(Mobile m, bool run, int range)
        {
            ...
            else if (!this.DoMove(m_Mobile.GetDirectionTo(m), true))
            {
            ...
            if (map != null)
            {
                ...
                foreach (Mobile m in eable)
                {
                    ...
                    if (acqType == FightMode.Aggressor || acqType == FightMode.Evil)
                    {
                        ...
                        for (int a = 0; !bValid && a < this.m_Mobile.Aggressors.Count; ++a)
                            bValid = (this.m_Mobile.Aggressors[a].Attacker == m);

                        for (int a = 0; !bValid && a < this.m_Mobile.Aggressed.Count; ++a)
                            bValid = (this.m_Mobile.Aggressed[a].Defender == m);
