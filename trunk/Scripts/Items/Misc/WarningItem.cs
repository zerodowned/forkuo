...
namespace Server.Items
{
    public class WarningItem : Item
    {
        ...
        public virtual void Broadcast(Mobile triggerer)
        {
            ...
            this.SendMessage(triggerer, this.OnlyToTriggerer, m_WarningString, m_WarningNumber);

            if (this.NeighborRange >= 0)
            {
                List<WarningItem> list = new List<WarningItem>();

                foreach (Item item in this.GetItemsInRange(NeighborRange))
                {
    ...
    public class HintItem : WarningItem
    {
        ...
        {
            this.SendMessage(from, true, m_HintString, m_HintNumber);
        }
