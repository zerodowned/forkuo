...
namespace Server.Items
{
    ...
    public class HouseRaffleStone : Item
    {
        ...
        public HouseRaffleState CurrentState
        {
            get
            {
                if (!this.m_Active && this.m_Winner != null)
        ...
        public bool IsExpired
        {
            get
            {
                if (this.CurrentState != HouseRaffleState.Completed)
                    return false;
        ...
        public HouseRaffleStone(Serial serial) : base(serial)
        {
        }
        ...
        public override void GetProperties(ObjectPropertyList list)
        {
            ...
            {
                list.Add(1060658, "winner\t{0}", this.m_Winner.Name); // ~1_val~: ~2_val~
            }
        }
        ...
        public override void OnSingleClick(Mobile from)
        {
            ...
            if (this.m_Active)
                this.LabelTo(from, 1060658, String.Format("Ends\t{0}", m_Started + m_Duration)); // ~1_val~: ~2_val~
            else if (this.m_Winner != null)
                this.LabelTo(from, 1060658, String.Format("Winner\t{0}", m_Winner.Name)); // ~1_val~: ~2_val~
        }
        ...
        {
            if (!this.m_Active || !from.CheckAlive())
                return;
        ...
        {
            if (this.Deleted || !from.CheckAlive() || this.HasEntered(from) || this.IsAtIPLimit(from))
                return;
        ...
        {
            if (!this.m_Active || this.m_Started + this.m_Duration > DateTime.Now)
                return;
            ...
            if (this.m_Region != null && this.m_Entries.Count != 0)
            {
                ...
                if (this.m_Winner != null)
                {
                    ...
                    this.m_Winner.SendMessage(MessageHue, "Congratulations, {0}!  You have won the raffle for the plot located at {1}.", this.m_Entries[winner].From.Name, this.FormatLocation());
                    ...
                    {
                        BankBox box = this.m_Winner.BankBox;

                        if (box.TryDropItem(this.m_Winner, this.m_Deed, false))
                        {
                        ...
                        else
                        {
                            ...
                            this.m_Winner.SendMessage(MessageHue, "As both your backpack and bank box are full, the writ of lease has been placed at your feet.");
                        }
                    }
                }
            }
        ...
        private class ActivateEntry : RaffleContextMenuEntry
        {
            ...
            public override void OnClick()
            {
                if (this.m_Stone.Deleted || this.m_From.AccessLevel < AccessLevel.Seer || this.m_Stone.Active || !this.m_Stone.ValidLocation())
                    return;

                this.m_Stone.Active = true;
            }
        }
