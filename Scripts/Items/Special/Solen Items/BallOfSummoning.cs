...
namespace Server.Items
{
    public class BallOfSummoning : Item, TranslocationItem
    {
        ...
        {
            this.LabelTo(from, 1054131, m_Charges.ToString() + (m_PetName.Length == 0 ? "\t " : "\t" + m_PetName)); // a crystal ball of pet summoning: [charges: ~1_charges~] : [linked pet: ~2_petName~]
        }
        ...
        private class PetSummoningSpell : Spell
        {
            ...
            public PetSummoningSpell(BallOfSummoning ball, Mobile caster) : base(caster, null, m_Info)
            {
