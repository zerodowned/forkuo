...
namespace Server.Guilds
{
    public abstract class BaseGuildListGump<T> : BaseGuildGump
    {
        ...
        public BaseGuildListGump(PlayerMobile pm, Guild g, List<T> list, IComparer<T> currentComparer, bool ascending, string filter, int startNumber, InfoField<T>[] fields) : base(pm, g)
        {
        ...
        public override void PopulateGump()
        {
            ...
            if (this.WillFilter)
            {
                ...
                {
                    if (!this.IsFiltered(list[i], m_Filter))
            ...
            this.AddTextEntry(135, 80, 375, 30, 0x481, 1, m_Filter);
            ...
            if (this.m_Ascending)
                for (int i = this.m_StartNumber; i < this.m_StartNumber + itemsPerPage && i < this.m_List.Count; i++)
                    this.DrawEntry(m_List[i], i, itemNumber++);
        ...
        public virtual void DrawEntry(T o, int index, int itemNumber)
        {
            ...
            for (int j = 0; j < this.m_Fields.Length; j++)
            {
                ...
                this.AddHtmlText(70 + width, 141 + itemNumber * 28, f.Width, 20, this.GetValuesFor(o, m_Fields.Length)[j], false, false);
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
                ...
                case 5:	//Filter
                    {
                        ...
                        pm.SendGump(this.GetResentGump(player, guild, m_Comparer, m_Ascending, (t == null) ? "" : t.Text, 0));
                        break;
                    }
                case 6: //Back
                    {
                        pm.SendGump(this.GetResentGump(player, guild, m_Comparer, m_Ascending, m_Filter, m_StartNumber - itemsPerPage));
                        break;
                    }
                case 7:	//Forward
                    {
            ...
            if (id >= 100 && id < (100 + this.m_Fields.Length))
            {
                ...
                pm.SendGump(this.GetResentGump(player, guild, comparer, m_Ascending, m_Filter, 0));
            }
            else if (id >= 200 && id < (200 + this.m_List.Count))
            {
                pm.SendGump(this.GetObjectInfoGump(player, guild, m_List[id - 200]));
            }
        }
        ...
        {
            this.player.SendGump(this.GetResentGump(player, guild, m_Comparer, m_Ascending, m_Filter, m_StartNumber));
        }
    }
