...
namespace Server.Mobiles
{
    ...
    public partial class BaseCreature : Mobile, IHonorTarget
    {
        ...
        public virtual bool IsBondable
        {
            get
            {
                return (BondingEnabled && !this.Summoned);
            }
        }
        ...
        {
            this.PlaySound(BreathAngerSound);
        }

        public virtual void BreathPlayAngerAnimation()
        {
            this.Animate(BreathAngerAnimation, 5, 1, true, false, 0);
        }
        ...
        {
            this.PlaySound(BreathEffectSound);
        }
        ...
        */
        ...
        {
            if (this.IsParagon && !this.GivesMLMinorArtifact)
            {
                if (suffix.Length == 0)
                    suffix = "(Paragon)";
                else
        ...
        public virtual bool OnGoldGiven(Mobile from, Gold dropped)
        {
            ...
            {
                if (this.Teach(m_Teaching, from, dropped.Amount, true))
                {
                    dropped.Delete();
                    return true;
                }
        ...
        public virtual bool CheckFoodPreference(Item f)
        {
            ...
            if (this.CheckFoodPreference(f, FoodType.Gold, m_Gold))
                return true;
        ...
        {
            if (!this.IsDeadPet && this.Controlled && (this.ControlMaster == from || this.IsPetFriend(from)))
            {
                Item f = dropped;
        ...
        public virtual void OnGaveMeleeAttack(Mobile defender)
        {
            ...
            if (p != null && this.HitPoisonChance >= Utility.RandomDouble())
            {
                ...
                if (this.Controlled)
                    this.CheckSkill(SkillName.Poisoning, 0, Skills[SkillName.Poisoning].Cap);
            }
        ...
        */
        ...
        public override void OnMovement(Mobile m, Point3D oldLocation)
        {
            ...
            if (this.ReacquireOnMovement || this.m_Paragon)
                this.ForceReacquire();
        ...
        public override bool OnBeforeDeath()
        {
            ...
            if (!this.Summoned && !this.NoKillAwards && !this.IsBonded)
            {
                if (treasureLevel >= 0)
                {
                    if (this.m_Paragon && Paragon.ChestChance > Utility.RandomDouble())
                        this.PackItem(new ParagonChest(this.Name, treasureLevel));
                    else if ((this.Map == Map.Felucca || this.Map == Map.Trammel) && TreasureMap.LootChance >= Utility.RandomDouble())
                        this.PackItem(new TreasureMap(treasureLevel, Map));
                }
        ...
        */
        ...
        {
            if (!this.Alive || this.Map == Map.Internal || !this.CanBeBeneficial(patient, true, true) || patient.Map != this.Map || !this.InRange(patient, HealEndRange))
            {
                this.StopHeal();
                return;
            }
        ...
        public virtual void OnThink()
        {
            ...
            if ((this.CanHeal || this.CanHealOwner) && this.Alive && !this.IsHealing && !this.BardPacified)
            {
                ...
                if (owner != null && this.CanHealOwner && DateTime.Now >= this.m_NextHealOwnerTime && this.CanBeBeneficial(owner, true, true) && owner.Map == this.Map && this.InRange(owner, HealStartRange) && this.InLOS(owner) && owner.Hits < this.HealOwnerTrigger * owner.HitsMax)
                {
